...
load('result_base_testing.RData')
#setwd('//Users/TScott/Google Drive/elwha/PSJ_Submission/Version3')
#load('only_model_results.RData')

load('result_base_testing.RData')

tempA<-ggs(mod_baseA$sample)
tempB<-ggs(mod_baseB$sample)
tempC<-ggs(mod_baseC$sample)
tempNull <- ggs(mod_baseNull$sample)
...
ggdudens = ggplot(data=tempA,aes(colour=as.character(Chain),x=value))+
