...
library(reshape2)
library(mail)
#search.ergmTerms(categories=c('valued'))
#setwd('H:/passtosim')
...
mod_base <-
  ergm(form_base,
       ...
       control=control.ergm(init=c(geo.init, rep(0, npar)),
                            MCMLE.maxit=36,MCMC.runtime.traceplot=F,seed=24,
...
save.image('result_base5_nll.RData')

sendmail("tyler.andrew.scott@gmail.com", "R notice: runmod_base5", "runmod_base5 finished.")
# mod_indpart3<-
