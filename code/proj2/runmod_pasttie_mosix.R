...
npar = length(summary(form_pasttie,response='TVAL'))-1
modpast_pasttie <-
  ergm(form_pasttie,
       response="TVAL", reference=~DiscUnif(0,3),
               control=control.ergm(init=c(geo.init, rep(0, npar)),
...
#   nodecov('allpartdiv7')
# 
# npar = length(summary(form_allpart,response='TVAL'))-1
# 
# mod_allpart <-
#   ergm(form_allpart,
#        response="TVAL", reference=~DiscUnif(0,3),
...
# 
# npar = length(summary(form_allpartquad,response='TVAL'))-1
# 
# mod_allpartquad <-
#   ergm(form_allpartquad,
#        response="TVAL", reference=~DiscUnif(0,3),
...
#                             MCMC.prop.args=list(p0=0.5)),eval.loglik=F,verbose=T)
# 
# save.image('result_allpartmodquad.RData')

proc.time() - t0
stopCluster(cluster)
#print(modpast_allpart7)

mpi.exit()

sendmail("tyler.andrew.scott@gmail.com", "R notice: runmod_pasttie", "runmod_pastiepart finished on mosix")
