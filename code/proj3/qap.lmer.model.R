...
library(lavaan)
# access.df$Response.Factor = access.df$Response.Option
# access.df$Response.Factor[access.df$Response.Factor=="na"] = NA
# access.df$Response.Factor[access.df$Response.Factor=="don't know"] = 'neither agree nor disagree'
...
perm.mean.ests = data.frame(param = character(0),mean= numeric(0),dep.var= character(0))
param.ests = data.frame(param = character(0),ests = character(0),dep.var=character(0))



### Estimate posteriors
param.ests = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
temp.data = access.df %>% dplyr::filter(Short == i)
temp.mod = inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
                family='binomial',Ntrials=1,data=temp.data)
temp.param.ests =   data.frame(param =rownames(temp.mod$summary.fixed),
                               mean.ests = temp.mod$summary.fixed$mean,
                               lower.ests = temp.mod$summary.fixed$`0.025quant`, 
                               upper.ests = temp.mod$summary.fixed$`0.975quant`,
                               dep.var = i)
#param.ests = rbind(param.ests,temp.param.ests)
}


N = 1000

perm.mean.ests1 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
    print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
                        family='binomial',Ntrials=1,data = x), mc.preschedule = TRUE)
  #mod.sums = lapply(mod.storage.temp,summary)
  params = lapply(mod.storage.temp, function(x) x$summary.fixed['mean'])
temp.perm.ests = data.frame(param = rep(rownames(params[[1]]),length(params)), mean = unlist(params),
                       dep.var = i)
temp.perm.ests
#perm.mean.ests = rbind(perm.mean.ests,temp.perm.ests)
}

perm.mean.ests2 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests3 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests4 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests5 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests6 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests7 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests8 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests9 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
perm.mean.ests10 = foreach(i = unique(access.df$Short), .combine=rbind) %dopar% {
  print(i)
  temp.data = access.df %>% dplyr::filter(Short == i)
  perm.data = plyr::rlply(N,temp.data %>% mutate(Increase = sample(Increase)))
  mod.storage.temp = mclapply(perm.data,function(x) inla(Increase ~ 1 + Participation + Number.Of.Groups + diff.from.group.brok.median + brokerage.score + bimodal.brokerage.score + ties.to.other.members + f(uq.ID,model='iid') + f(Group,model='iid'),
...
#rm(mod.storage.temp);rm(mod.sums);rm(perm.data);rm(params)

#save results
if(remote)
{save.image('temp.qap.results.RData')}
if(!remote)
{save.image('code/proj3/temp.qap.results.RData')}
